# env = Environment()
# env.pcd_to_binary_image(pcd) #转2dimage
# env.thin()
...
class Environment:
    def __init__(self):
        ...
        self.classification_model = torch.load('/home/pthahnil/CCH_Model/realworld_model_epoch_29.pt',
                                               map_location=torch.device('cpu'))
    ...
    def pcd_to_binary_image(self, pcd):
        ...
        chosen_idx = np.logical_and(pcd[:, 1] < 0.02, pcd[:, 1] > -0.01)
        pcd_chosen_in_body = pcd[chosen_idx, :]
        # # 选取z0.01-1距离的点
        chosen_idx = np.logical_and(pcd_chosen_in_body[:, 2] > -1, pcd_chosen_in_body[:, 2] < -0.01)
        chosen_x = pcd_chosen_in_body[chosen_idx, 0]
        chosen_z = pcd_chosen_in_body[chosen_idx, 2]

        self.img_binary = np.zeros((100, 100)).astype('uint8')
        if np.any(chosen_x):
            self.pcd_2d = pcd_chosen_in_body[chosen_idx, 0::2]

            # chosen_x, chosen_z = self.line_ground_calibrate(chosen_x, chosen_z)

            # # 和z=0,y=1对齐
            x_min = np.min(chosen_x)
            z_min = np.min(chosen_z)

            chosen_x = chosen_x + (0.01 - x_min)
            chosen_z = chosen_z + (0.01 - z_min)

            # 只取出最前方1m^2内的点
            chosen_idx = np.logical_and(np.logical_and(chosen_x > 0, chosen_x < 1),
                                        np.logical_and(chosen_z > 0, chosen_z < 1))
            chosen_x = chosen_x[chosen_idx]
            chosen_z = chosen_z[chosen_idx]
            pixel_x = np.floor(100 * chosen_x).astype('int')
            pixel_z = np.floor(100 * chosen_z).astype('int')
            self.img_binary = np.zeros((100, 100)).astype('uint8')
            for i in range(np.size(pixel_x)):
                self.img_binary[99-pixel_z[i],pixel_x[i]] = 255

        else:
            self.pcd_2d = np.zeros([len(chosen_x), 2])

    def plot_binary_image(self):
        plt.imshow(self.img_binary, cmap='gray')

        plt.title('Binary Image')
        plt.axis('off')
        plt.savefig('binary_image.png')  # 保存图像到文件
        plt.close()  # 关闭图像
