...
if __name__ == "__main__":
    ser = Motor.init_motor(port=ser_port_motor)
    Motor.send_signals_to_motor(ser, np.array([0, 0, 0, 0, 5, 0.1, 3, 0.1]))
    ...
    for i in range(np.shape(six_force_smooth_buffer)[0]):
        ...
        q_ankle_smooth_buffer = fifo_mat(q_ankle_smooth_buffer,q_qv[2])
    ...
    try:
        while True:
            ...
            phase = update_phase(phase=phase, 
                                 My_min = My_min, 
                                 Fz_max = Fz_max, 
                                 Fz_min = Fz_min, 
                                 q_k_buf=q_knee_smooth_buffer,
                                 q_a_buf=q_ankle_smooth_buffer)
            desired_k, desired_b, desired_qe = update_impedance_paras(phase)
            cmd = cal_stance_control_command(desired_k, desired_b, desired_qe)
            Motor.send_signals_to_motor(ser, q_qv_d=cmd)
            ...
            tau_k = i_t[1]*6.1/1000*0.11*50
            tau_a = i_t[3]*6.1/1000*0.11*50
            print("\rphase ={}, tau_knee={}, tau_ankle={}".format(phase, np.round(tau_k,2), np.round(tau_a,2)), end="")
            time.sleep(8e-3)
    except KeyboardInterrupt:
        print("Over")